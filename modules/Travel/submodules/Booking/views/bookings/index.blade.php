        ...
        mixins.push({
            data () {
                return {
                    dataset: {
                        test: 1,
                        urls: {
                            edit: '{{ route('bookings.edit', 'null') }}',
                            show: '{{ route('bookings.show', 'null') }}',
                            destroy: '{{ route('bookings.destroy', 'null') }}',
                            api: {
                                destroy: '{{ route('api.bookings.destroy', 'null') }}',
                            },
                        },
                        bulk: {
                            destroy: { model: false }
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.bookings.all') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
