            ...
            <v-flex sm5 md4 xs12>
                <v-card class="elevation-1 mb-3">
                    <v-toolbar card class="transparent">
                        ...
                        <v-toolbar-title class="accent--text">{{ __('New Budget') }}</v-toolbar-title>
                    </v-toolbar>

                    <form action="{{ route('bookings.budgets.store') }}" method="POST">
                        {{ csrf_field() }}
                        <v-card-text>
                            <input type="hidden" name="budgetable_type" value="booking">
                            <v-text-field
                            ...
                            <v-text-field
                                ...
                                hint="{{ __('Will be used as a unique budget name.') }}"
                                label="{{ _('Code') }}"
                                name="code"
                            ></v-text-field>
                            <v-text-field
                ...
                <v-card class="mb-3 elevation-1">
                    <v-toolbar class="transparent elevation-0">
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                <form action="{{ route('budgets.many.destroy') }}" method="POST" class="inline">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <template v-for="item in dataset.selected">
                                        <input type="hidden" name="budgets[]" :value="item.id">
                                    </template>
                                    <v-btn
                                        flat
                                        icon
                    ...
                    >
                        ...
                        <template slot="items" scope="prop">
                            ...
                            <td width="100%">
                                <a :href="route(urls.budgets.edit, prop.item.id)">
                            ...
                            <td class="text-xs-center">
                                <v-menu bottom left>
                                    ...
                                    <v-list>
                                        <v-list-tile :href="route(urls.budgets.show, (prop.item.id))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile :href="route(urls.budgets.edit, (prop.item.id))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile @click="post(route(urls.budgets.api.clone, (prop.item.id)))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile
                                            @click="destroy(route(urls.budgets.api.destroy, prop.item.id),
                                            {
                                                '_token': '{{ csrf_token() }}'
                                            })">
                                            <v-list-tile-action>
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        budgets: {
                            api: {
                                clone: '{{ route('api.bookings.budgets.clone', 'null') }}',
                                destroy: '{{ route('api.bookings.budgets.destroy', 'null') }}',
                            },
                            show: '{{ route('budgets.show', 'null') }}',
                            edit: '{{ route('budgets.edit', 'null') }}',
                            destroy: '{{ route('budgets.destroy', 'null') }}',
                        },
                    },
            ...
            watch: {
                ...
                'dataset.searchform.query': function (filter) {
                    setTimeout(() => {
                        ...
                        this.api().search('{{ route('api.bookings.budgets.search') }}', query)
                            .then((data) => {
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.bookings.budgets.all') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
                ...
                post (url, query) {
                    ...
                    this.api().post(url, query)
                        .then((data) => {
                            self.get('{{ route('api.bookings.budgets.all') }}');
