            ...
            <v-flex sm5 md4 xs12>
                <v-card class="elevation-1 mb-3">
                    <v-toolbar card class="transparent">
                        <v-icon class="accent--text">fa-paperclip</v-icon>
                        <v-toolbar-title class="accent--text">{{ __('New Amenity') }}</v-toolbar-title>
                    </v-toolbar>
            ...
            <v-flex sm7 md8 xs12>
                <v-card class="mb-3 elevation-1">
                    <v-toolbar class="transparent elevation-0">
                        <v-toolbar-title class="accent--text">{{ __('Amenities') }}</v-toolbar-title>
                        <v-spacer></v-spacer>
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                <form action="{{ route('amenities.many.destroy') }}" method="POST" class="inline">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                    ...
                    >
                        ...
                        <template slot="items" scope="prop">
                            ...
                            <td width="100%">
                                <a :href="route(urls.amenities.edit, prop.item.id)">
                            ...
                            <td class="text-xs-center">
                                <v-menu bottom left>
                                    ...
                                    <v-list>
                                        <v-list-tile :href="route(urls.amenities.show, (prop.item.id))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile :href="route(urls.amenities.edit, (prop.item.id))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile @click="post(route(urls.amenities.api.clone, (prop.item.id)))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile
                                            @click="destroy(route(urls.amenities.api.destroy, prop.item.id),
                                            {
                                                '_token': '{{ csrf_token() }}'
                                            })">
                                            <v-list-tile-action>
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        amenities: {
                            api: {
                                clone: '{{ route('api.amenities.clone', 'null') }}',
                                destroy: '{{ route('api.amenities.destroy', 'null') }}',
                            },
                            show: '{{ route('amenities.show', 'null') }}',
                            edit: '{{ route('amenities.edit', 'null') }}',
                            destroy: '{{ route('amenities.destroy', 'null') }}',
                        },
                    },
            ...
            watch: {
                ...
                'dataset.searchform.query': function (filter) {
                    setTimeout(() => {
                        ...
                        this.api().search('{{ route('api.amenities.search') }}', query)
                            .then((data) => {
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.amenities.all') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
                ...
                post (url, query) {
                    ...
                    this.api().post(url, query)
                        .then((data) => {
                            self.get('{{ route('api.amenities.all') }}');
