        ...
        <v-layout row wrap>
            ...
            <v-flex sm5 md4 xs12>
                <v-card class="elevation-1 mb-3">
                    ...
                    <form action="{{ route('packups.budgets.store') }}" method="POST">
                        {{ csrf_field() }}
                        <v-card-text>
                            <input type="hidden" name="budgetable_type" value="packup">
                            <v-text-field
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        budgets: {
                            api: {
                                clone: '{{ route('api.packups.budgets.clone', 'null') }}',
                                destroy: '{{ route('api.packups.budgets.destroy', 'null') }}',
                            },
            ...
            watch: {
                ...
                'dataset.searchform.query': function (filter) {
                    setTimeout(() => {
                        ...
                        this.api().search('{{ route('api.packups.budgets.search') }}', query)
                            .then((data) => {
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.packups.budgets.all') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
                ...
                post (url, query) {
                    ...
                    this.api().post(url, query)
                        .then((data) => {
                            self.get('{{ route('api.packups.budgets.all') }}');
