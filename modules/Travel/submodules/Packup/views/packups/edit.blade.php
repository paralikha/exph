...
@section("content")
    <v-container fluid grid-list-lg>
        ...
        <form action="{{ route('packups.update', $resource->id) }}" method="POST">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
                ...
                <v-flex md9>
                    <v-card class="mb-3 elevation-1">
                        <v-toolbar card class="transparent">
                            <v-toolbar-title class="accent--text">{{ __('Edit Packup') }}</v-toolbar-title>
                            <v-spacer></v-spacer>
                        </v-toolbar>
                    ...
                    @include("Packup::interactives.map")
                </v-flex>

                <v-flex md3>
                    @include("Theme::cards.saving")

                    @include("Packup::cards.price")

                    @include("Theme::interactives.featured-image")

                    @include("Packup::interactives.managers")
