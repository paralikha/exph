        ...
        mixins.push({
            data () {
                return {
                    dataset: {
                        test: 1,
                        urls: {
                            edit: '{{ route('packups.edit', 'null') }}',
                            show: '{{ route('packups.show', 'null') }}',
                            destroy: '{{ route('packups.destroy', 'null') }}',
                            api: {
                                destroy: '{{ route('api.packups.destroy', 'null') }}',
                            },
                        },
                        bulk: {
                            destroy: { model: false }
            ...
            watch: {
                ...
                'dataset.searchform.query': function (filter) {
                    setTimeout(() => {
                        ...
                        this.api().search('{{ route('api.packups.search') }}', query)
                            .then((data) => {
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.packups.all') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
